...
impl<F: FieldExt> RangeInstructions<F> for RangeChip<F> {
    ...
    fn decompose(
        ...
        let decomposed = unassigned.decompose(number_of_limbs, limb_bit_len);
...
mod tests {
    ...
    use crate::{MainGateInstructions, Term, UnassignedValue};
    ...
    fn test_range_circuit() {
        ...
        let inputs = (2..20)
            .map(|number_of_limbs| {
                ...
                Input {
                    value: Some(Fp::from_u128((1 << bit_len) - 1)),
